...
for kotlin_version in $versions; do
  string_concat_modes=(inline)
  if [[ "$kotlin_version" != *-* ]]; then
    string_concat_modes=(inline indy indy-with-constants)
  fi

  for string_concat in "${string_concat_modes[@]}"; do
    echo "  - Kotlin $kotlin_version / StringConcat $string_concat"

    row="{\"kotlin\":\"$kotlin_version\",\"stringConcat\":\"$string_concat\"}"
    if [[ "$first" == true ]]; then
      rows="$row"
      first=false
    else
